    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        ...
        if self.opt.cexe:
            return self.decode(src_encode, src_mask,tgt, tgt_mask), \
                self.decode(torch.zeros_like(src_encode), src_mask,tgt, tgt_mask)
        else:
            return self.decode(src_encode, src_mask,
                                tgt, tgt_mask)
...
class TransformerModel(AttModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        if self.opt.cexe:
            out, out_interven = self.model(att_feats, seq, att_masks, seq_mask)
        else:
            out = self.model(att_feats, seq, att_masks, seq_mask)

        if self.opt.cexe:
            outputs = self.model.generator(out)
            outputs_tie = F.log_softmax(self.model.generator.proj(out) - self.model.generator.proj(out_interven), dim=-1)
        else:
            outputs = self.model.generator(out)
        if self.opt.cexe:
            return outputs, outputs_tie
        else:
            return outputs
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        """
        state = [ys.unsqueeze(0)], ph: place-holder
        """
        if state is None:
            ys = it.unsqueeze(1)
        else:
